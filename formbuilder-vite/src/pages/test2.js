...
function EditableTextModuleTitle({
  ...
  switch (theFontSize) {
    case "h1":
      // TODO:
      // CHQ: GEmini AI suggestion: add property to wherever contenteditable is set to isEditing
      // dangerouslySetInnerHTML={{ __html: theText }}
      return (
        <h1
          className={isEditing ? "hasBorder" : "noBorder"}
...
function SmallTextFieldAlt({ isVisible, thisText, setThisText }) {
  ...
  return (
    <div className="testMe">
      <h1 className="hasBorder" contentEditable={isVisible}>
        {" "}
        {setThisText}{" "}
      </h1>
